...
function normalizeText(input, isHTML = false) {
	// Usage examples:
	// const normalized = normalizeText("Résumé with accènts", false);
	// const normalizedHTML = normalizeText("<p>Café au lait</p>", true);
	
	
  // Comprehensive mapping of diacritics/accents to standard Latin characters
  const charMap = {
    ...
    '€': 'EUR', '£': 'GBP', '¥': 'JPY', '₩': 'KRW', '₽': 'RUB', '¢': 'c',
    '©': '(c)', '®': '(r)', '™': '(tm)',
    '¿': '?', '¡': '!', '«': '"', '»': '"', '‹': "'", '›': "'",
    '–': '-', '—': '-', '•': '*'
  };

  // Check for String.normalize support
  ...
  const normalizeContent = str => {
    if (hasNormalize) {
      // Use native normalization for better performance
      return str.normalize('NFD').replace(/[\u0300-\u036f]/g, '').normalize('NFC');
    } else {
      // Fallback to character map replacement
  ...
  // For HTML content, preserve tags while only replacing text content
  ...
  if (isBrowser) {
    ...
    // This is not as robust but works for basic HTML without complex nesting
